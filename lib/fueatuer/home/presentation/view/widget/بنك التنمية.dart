...
class Bankaltanmia extends StatefulWidget {
  const Bankaltanmia({super.key});

  @override
  State<Bankaltanmia> createState() => _BankaltanmiaState();
}

class _BankaltanmiaState extends State<Bankaltanmia> {
  final _nameController = TextEditingController();
  final _phoneController = TextEditingController();
  final _streetController = TextEditingController(); // حقل اسم الشارع
  final _levelController = TextEditingController(); // حقل المستوى الدراسي
  File? _imageFile;
  ...
  Future<void> _loadChildrenDetails() async {
    ...
    final data = prefs.getString('childrenDetails');
    if (data != null) {
      setState(() {
  ...
  Future<void> _saveChildrenDetails() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString('childrenDetails', jsonEncode(_childrenDetails));
  }
  ...
  void _saveChildDetails() async {
    ...
    final street = _streetController.text; // الحصول على اسم الشارع
    final level = _levelController.text; // الحصول على المستوى الدراسي
    ...
    if (_editingIndex != null) {
      setState(() {
        _childrenDetails[_editingIndex!] = {
          'name': name,
          'phone': phone,
          'street': street, // إضافة اسم الشارع
          'level': level, // إضافة المستوى الدراسي
  ...
  void _editChildDetails(int index) {
    ...
    setState(() {
      ...
      _streetController.text = child['street']; // تحميل اسم الشارع
      _levelController.text = child['level']; // تحميل المستوى الدراسي
      _imageFile = null;
      _editingIndex = index;
    });
  ...
  void _resetFields() {
    ...
    _streetController.clear(); // مسح حقل اسم الشارع
    _levelController.clear(); // مسح حقل المستوى الدراسي
    _imageFile = null;
    _editingIndex = null;
  }
